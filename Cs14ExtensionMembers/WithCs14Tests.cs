...
public static class StringExtensions
{
    ...
    extension(string?)
    {
        public static string Create(string pattern, int count)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(count), "Pattern must be positive");
            }
            else
            {
...
public class WithCs14Tests
{
    ...
    [TestCase("foo", 0, typeof(ArgumentOutOfRangeException))]
    public void ThrowsForInvalidCreateStringArguments(string? pattern, int count, Type expected)
    {
        Should.Throw(() => string.Create(pattern, count), expected);
    }
